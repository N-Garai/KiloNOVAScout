...
class KilonovaScoutTools:
    ...
    def query_glade_catalog(self, skymap: HealpixSkymap) -> List[Galaxy]:
        ...
        # using the skymap data. For hackathon demo, we return mock galaxies.
        mock_galaxies = [
        ...
        # S_i = (P_spatial_i)^alpha * (L_K_i / L*)^beta * exp(-gamma * X_i) * (1 - f_cloud)^delta * B_i
        from config.scoring_weights import load_scoring_weights
        ...
        try:
            ...
            f_cloud = weather.cloud_cover_percent / 100.0
        except:
        ...
        for galaxy in mock_galaxies:
            ...
            # Composite score from PRD formula
            composite_score = (
                (P_spatial ** weights.get('spatial_prior', 1.0)) *
                (L_ratio ** weights.get('galaxy_mass_prior', 0.6)) *
                np.exp(-weights.get('airmass_penalty', 0.2) * X_i) *
                ((1 - f_cloud) ** weights.get('cloud_cover_penalty', 0.5)) *
                (weights.get('grb_coincidence_boost', 3.0) if B_i > 1.0 else 1.0)
            )
