...
def build_report_markdown(record, weights: Optional[Dict[str, float]] = None) -> str:
    ...
    lines.append("")
    lines.append(profile.get('strategy', ''))
    lines.append("")
    ...
    lines.append("---")
    lines.append(f"*© KiloNovaScout · Autonomous Multi-Messenger Targeting Agent · {now} · Report {record.run_id}*")
    return "\n".join(lines)
...
def build_report_latex(record, weights: Optional[Dict[str, float]] = None) -> str:
    ...
    tex.append(r"\bottomrule\end{tabular}")
    tex.append(r"\vfill")
    tex.append(r"\footnotesize \textcopyright{} KiloNovaScout --- Autonomous Multi-Messenger Targeting Agent --- " + _tex_escape(now) + " --- Report " + _tex_escape(str(record.run_id)))
